...
PRED_TREE_FMT = imset.lower()+'_%06d_prediction_tree.mat'
PRED_MAT_NAME = 'pred_img'
SM_FMT = 'test_%06d_softmax.mat'
...
if __name__ == '__main__':
	for im_idx in range(1, num_img+1):
		...
		softmax = exp_logits / np.expand_dims(np.sum(exp_logits, axis=2), axis=2)
		#softmax = resize_func(softmax, h, w).astype(np.float32)
			
		labels = np.argmax(softmax, axis=2).astype(np.uint8)
		savemat(join(OUTPUT_PATH, PRED_FMT % im_idx),
				{PRED_MAT_NAME: labels})
				
		tree_labels = convert_to_tree_labelmap(labels, mapping)
		savemat(join(OUTPUT_PATH, PRED_TREE_FMT % im_idx),
				{PRED_MAT_NAME: tree_labels})
				
		# I was getting memory errors
		sm_im = np.zeros((h, w), dtype=np.float32)
		for i, row in enumerate(labels):
			for j, idx in enumerate(row):
				sm_im[i, j] = softmax[i, j, idx]
		
		savemat(join(OUTPUT_PATH, SM_FMT % im_idx),
				{SM_MAT_NAME: sm_im})
