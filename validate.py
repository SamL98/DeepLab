...
def validate_calibration(slice_file, num_fg_pix, per_class_counts):
	...
	for i, slc in enumerate(slices[1:]):
		...
		assert perc_diff < 0.1, 'Non-consistent num_fg_pix across slices: %d vs %d for slice %d' % (slc0_num_fg_pix, slc_num_fg_pix, i+1)

	''' Validate item #2 '''
	perc_diff = abs(num_fg_pix - slc0_num_fg_pix) / num_fg_pix
	assert perc_diff < 0.1, 'Non-consistent num_fg_pix on slice 0: %d vs %d' % (num_fg_pix, slc0_num_fg_pix)
	...
	for slc in slices:
		for node in slc:
			...
			assert perc_diff < 0.1, 'Non-consistent num_pix for %s node: %d vs %d' % (node.name, init_num_pix, final_num_pix)
...
def validate_inference(slice_file, imset, name, conf_thresh):
	''' Validate item #1 '''
	slices = read_slices(slice_file)

	for idx in range(1, num_img_for(imset)+1):
		logits = load_logits(imset, idx, reshape=False)
		calib_pred = load_calib_pred(imset, idx, conf_thresh, name).ravel()
